...
class LPR:
    ...
    def apply_adaptive_threshold(self, img):
        return cv2.adaptiveThreshold(
            img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 7, 13
        )
    ...
    def filter_candidates(self, contours):
        ...
        for cnt in contours:
            ...
            if (
                np.isclose(aspect_ratio, self.ratio, atol=0.7)
                and (self.max_w > w > self.min_w)
                and (self.max_h > h > self.min_h)
            ):
    ...
    #MÉTODO MODIFICADO PARA DEVOLVER EL PROCESO
    def read_license(self, img, psm=7):
        alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
        options = "-c tessedit_char_whitelist={}".format(alphanumeric)
        options += " --psm {}".format(psm)

        pasos = {}  #Aquí guardamos todo el proceso

        gray = self.grayscale(img)
        pasos["1 - Grises"] = gray

        thresh = self.apply_threshold(gray)
        pasos["2 - Threshold"] = thresh
